...
def custom_scaled_dot_product_attention(q, k, v, attn_mask=None):
    d_k = q.size(-1)
    # FIX: Use math.sqrt instead of creating a tensor (avoids device mismatch)
    import math
    scale = math.sqrt(d_k)
...
class CustomMultiheadAttention(nn.Module):
    ...
    def forward(self, q, k, v, need_weights=False, attn_mask=None):
        # Input: q, k, v are (seq_len, batch, embed_dim) - PyTorch's default format
        tgt_len, bsz, embed_dim = q.shape
        src_len = k.shape[0]

        # Project Q, K, V
        q = self.Q(q)  # (tgt_len, bsz, embed_dim)
        k = self.K(k)  # (src_len, bsz, embed_dim)
        v = self.V(v)  # (src_len, bsz, embed_dim)
